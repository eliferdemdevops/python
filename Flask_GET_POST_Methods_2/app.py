...
# When the user comes directly "/calc" path, "Since this is a GET requests, LCM has not been calculated" string returns to the
@app.route("/calc", methods = ["GET", "POST"])
def calculate():
    if request.method == "POST":
        num1 = request.form.get("number1")
        num2 = request.form.get("number2")
        return render_template("result.html", result1=num1, result2=num2, lcm=lcm(int(num1),int(num2)), developer_name= "Elif")
    else:
